...
// Solution code here...
  let newLongString = ' ';
  for (let str of arr ){
    if(str.length > newLongString.length){
      newLongString = str; }
  }
  return arr.indexOf(newLongString);
};
...
const firstLetters = (arr) => {
  ...
  for(let str of arr){
    firstLettersString.push(str.slice(0,1));
  }
...
const allHappy = (arr) => {
  // Solution code here...
  let beHappy = arr.filter(str => /:\)/.test(str));
  if(beHappy.length === arr.length){
    return true;
  }
  return false;
};
