...
function transformToLis(obj){
  // Solution code here...
  const liValues = Object.values(obj);
  const liKeys = Object.keys(obj);
  const newArray = [];
  for ( let s= 0; s < liKeys.length; s++) {
    newArray.push(`<li>${liKeys[s]}: ${liValues[s]}</li>`);
  }
  return newArray;
...
const count = (target, input) => {
  // Solution code here...
  let fourSum = input.flat();
  let newNum = fourSum.reduce((acc, val) => {
...
const totalSum = (input) => {
  // Solution code here...
  let newOne = input.flat();
  let finalTotal = 0;
  for( let j = 0; j < newOne.length; j++) {
    finalTotal = finalTotal + newOne[j];
  }
  return finalTotal;
};
...
let findMaleAndFemale = (data) => {
  // Solution code here...
  const maleandFemaleArray = [];
  for (let g = 0; g < data.length; g++) {
    if (data[g].gender === 'male' || data[g].gender === 'female') {
      maleandFemaleArray.push(data[g].name);
    }
  }
  let maleandFemaleString = `${maleandFemaleArray[0]}`;
  for (let g = 1; g < maleandFemaleArray.length; g++) {
    maleandFemaleString = (maleandFemaleString + ` and ${maleandFemaleArray[g]}`);
  }
...
let findShortest = (data) => {
  // Solution code here...
  let shortStuff = data.reduce((acc, val) => {
    return (val.height > acc.height ? val : acc);
  }, data[0]);
  let shortStuffName = shortStuff.name;
  return shortStuffName;
};
