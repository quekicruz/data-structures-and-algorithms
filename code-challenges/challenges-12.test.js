...
const replaceZeros = (string) => {
  // Solution code here...
  const newRegex = /0/g;
  const replacedString = string.replace(newRegex, 'zero');
...
const validatePin = (pin) => {
  ...
  const giveFalse = pinRegex.test(pin);
  return giveFalse;
};
...
const hasNumber = (string) => {
  ...
  const giveFalse = numberRegex.test(string);
  return giveFalse;
...
const validateEmail = (email) => {
  // Solution code here...\
  const emailRegex = /^[A-za-z0-9]+\.?[A-za-z0-9]+\@[A-za-z0-9]+\.(net|com|org)$/g;
  const giveFalse = emailRegex.test(email);
  return giveFalse;
